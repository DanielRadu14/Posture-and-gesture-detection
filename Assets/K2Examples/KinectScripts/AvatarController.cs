...
//            jointPosition.y - offsetPos.y + kinectManager.sensorHeight,  //!! this should be better investigated .. 
    ...
    {	
        ...
        frameData.jointType = new string[bones.Length];
		...
		{
            frameData.rootPosition = MoveAvatar(UserID);
		}
		...
		for (var boneIndex = 0; boneIndex < bones.Length; boneIndex++)
		{
			...
			if(boneIndex2JointMap.ContainsKey(boneIndex))
			{
                ...
                frameData.quaternions[boneIndex] = TransformBone(UserID, joint, boneIndex, !(mirroredMovement ^ flipLeftRight));
                frameData.jointType[boneIndex] = joint.ToString();
            }
			else if(specIndex2JointMap.ContainsKey(boneIndex))
			{
				...
				if(alJoints.Count >= 2)
				{
                    ...
                    frameData.quaternions[boneIndex] = TransformSpecialBone(UserID, alJoints[0], alJoints[1], boneIndex, baseDir, !(mirroredMovement ^ flipLeftRight));
                    frameData.jointType[boneIndex] = alJoints[0].ToString();
                }
			}
		}
    ...
    /// <summary>
	/// Updates the avatar each frame using Recorded Data.
	/// </summary>
	public void UpdateAvatarFromFile()
    {
        /*if (manageSpeedMode())
        {
            return;
        ...
        if (RecordingManager.currentFrame < RecordingManager.framesCount)
        {
            RecordingManager.FrameData? frameData = RecordingManager.frames[RecordingManager.currentFrame++];

            Quaternion[] quaternionData = frameData.Value.quaternions;
            Vector3 rootPos = frameData.Value.rootPosition;

            for (var boneIndex = 0; boneIndex < bones.Length; boneIndex++)
            {
                if (bones[boneIndex] != null)
                {
                    bones[boneIndex].rotation = quaternionData[boneIndex];
                }
            }
        }
    }
...
//		}
    ...
    protected Quaternion TransformBone(Int64 userId, KinectInterop.JointType joint, int boneIndex, bool flip)
    {
	...
	protected Quaternion TransformSpecialBone(Int64 userId, KinectInterop.JointType joint, KinectInterop.JointType jointParent, int boneIndex, Vector3 baseDir, bool flip)
	{
		Transform boneTransform = bones[boneIndex];
		if(boneTransform == null || kinectManager == null)
            return Quaternion.identity;
...
//			return;
	...
	protected Vector3 MoveAvatar(Int64 UserID)
	{
