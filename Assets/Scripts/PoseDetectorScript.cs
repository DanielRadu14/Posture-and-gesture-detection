...
public class PoseDetectorScript : MonoBehaviour 
{
    ...
    //all considered joints
    private const int SpineShoulder = 0;
    private const int ShoulderLeft = 1;
    private const int ShoulderRight = 2;
    private const int Neck = 3;
    private const int Head = 4;

    // weights for each bone - used for computing the similarity between poses
    private static Dictionary<int, float> weights = new Dictionary<int, float>()
    {
        {SpineShoulder, 1.0f},
        {ShoulderLeft, 1.0f},
        {ShoulderRight, 1.0f},
